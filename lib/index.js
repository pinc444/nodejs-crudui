...
class CrudUI {
  ...
  _generateForm(columns, row = {}, table, mode = 'new', error = '') {
    ...
    columns.forEach(col => {
      ...
      if (columnConfig.customRenderEdit && typeof columnConfig.customRenderEdit === 'function') {
        ...
        // Format date values for HTML5 date inputs
        if ((columnConfig.dateColumn || this._isDateColumn(col)) && value && value !== '0000-00-00' && value !== '0000-00-00 00:00:00') {
          try {
        ...
        html += `<input type="${inputType}" name="${col.Field}" value="${this._escapeHtml(formattedValue)}" ${isReadonly ? 'readonly' : ''}/>`;
      }
    });
  ...
  _generateViewEditForm(columns, row = {}, table, id) {
    ...
    columns.forEach(col => {
      ...
      if (columnConfig.customRenderView && typeof columnConfig.customRenderView === 'function') {
        ...
        html += `<input type="${inputType}" name="${col.Field}" value="${this._escapeHtml(formattedValue)}" readonly class="view-input"/>`;
      }
    });
      ...
      if (value && value !== '0000-00-00' && value !== '0000-00-00 00:00:00') {
        try {
          ...
          if (typeof value === 'string') {
            // Replace any non-standard separators and handle MySQL date formats
  ...
  _generateDateFilters(columns) {
    ...
    let html = '<button type="button" class="btn edit-btn" onclick="toggleDateFilters()">Date Filter</button>';
    html += '<div class="date-filters" id="date-filters" style="display:none;">';
    ...
    html += '<button type="button" onclick="clearDateFilter()">Clear</button>';
    html += '</div>';
